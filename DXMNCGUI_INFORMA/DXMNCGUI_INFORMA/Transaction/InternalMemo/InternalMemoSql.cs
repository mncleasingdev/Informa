...
namespace DXMNCGUI_INFORMA.Transaction.InternalMemo
{
    public class InternalMemoSql : InternalMemoDB
    {
        ...
        public override DataTable LoadDataKontrak()
        {
            ...
            sQuery += "case a.OS_PERIOD when 0 then NULL else a.LAST_DUEDATE end[NEXT DUEDATE], OVERDUE from LS_AGREEMENT a \n";
            ...
            sQuery += "where a.CONTRACT_STATUS IN('GOLIVE','TERMINATE') \n";
        ...
        public override DataTable LoadDataDeviasiMemo()
        {
            myDeviasiMemoTable.Clear();

            sQuery = "select DocKey, DocNo, MemoTo, MemoCC,  MemoPerihal, MemoFrom from InternalMemo where DocType = 'MEMO DEVIASI' and Status = 'APPROVE' and DeviasiTBO is null";

            myLocalDBSetting.LoadDataTable(myDeviasiMemoTable, sQuery, true);
            DataColumn[] keyHeader = new DataColumn[1];
            keyHeader[0] = myDeviasiMemoTable.Columns["DocKey"];
            myDeviasiMemoTable.PrimaryKey = keyHeader;
            return myDeviasiMemoTable;
        }
        ...
        public override DataTable LoadDataDetailAsset(string scontract)
        {
            ...
            sQuery = "select LS_ASSETVEHICLEID, 0[DocKey], 0[Seq], Description[AssetDesc], ISNULL(VHCCHASS,'-')[NoRangka], ISNULL(SERIALNO,'-')[NoMesin], VHCYEAR[Tahun], 'F'[IsApprove], NULL[ApproveDate]  from [dbo].[LS_ASSETVEHICLE] where LSAGREE=?";
        ...
        public override DataTable LoadApprovalList(string strUserID)
        {
            ...
            if (strUserID == "2009023")
            { sQuery = "select a.*, b.MemoPerihal from WorkList a left join InternalMemo b on a.SourceKey = b.DocKey WHERE a.DocType <> 6"; }
            else
            { sQuery = "select a.*, b.MemoPerihal from WorkList a left join InternalMemo b on a.SourceKey = b.DocKey WHERE a.DocType <> 6 AND a.UserKey=?"; }
        ...
        protected override DataSet LoadData(long headerid, string docno, string debiturname)
        {
            ...
            SqlConnection mySmileconn = new SqlConnection(myDBSetting.ConnectionString);
            ...
            DataTable myHistoryStateTable = new DataTable();
            DataTable myDetailDeviasiTable = new DataTable();
            DataTable myDetailExceptionTable = new DataTable();
            ...
            //string sSQLDetailPelepasanCrossColl = @"exec dbo.sp_INFORMA_GetDetailPelepasanCrossColl @DocKey";
            ...
            string sSQLHistoryState = @"select * from InternalMemoStateHistory where dockey = @DocKey order by DateState desc";
            string sSQLDetailDeviasi = "SELECT * FROM dbo.InternalMemoDetailDeviasi WHERE DocKey=@DocKey ORDER BY Seq";
            string sSQLDetailException = "SELECT * FROM dbo.InternalMemoDetailException WHERE DocKey=@DocKey ORDER BY Seq";
            ...
            using (SqlCommand cmdhistorystate = new SqlCommand(sSQLHistoryState, myconn))
            {
                SqlDataAdapter adapter = new SqlDataAdapter(cmdhistorystate);
                cmdhistorystate.Parameters.Add("@DocKey", SqlDbType.BigInt);
                cmdhistorystate.Parameters["@DocKey"].Value = headerid;
                adapter.Fill(myHistoryStateTable);
            }

            using (SqlCommand cmddetaildeviasi = new SqlCommand(sSQLDetailDeviasi, myconn))
            {
                SqlDataAdapter adapter = new SqlDataAdapter(cmddetaildeviasi);
                cmddetaildeviasi.Parameters.Add("@DocKey", SqlDbType.BigInt);
                cmddetaildeviasi.Parameters["@DocKey"].Value = headerid;
                adapter.Fill(myDetailDeviasiTable);
            }

            using (SqlCommand cmddetailexception = new SqlCommand(sSQLDetailException, myconn))
            {
                SqlDataAdapter adapter = new SqlDataAdapter(cmddetailexception);
                cmddetailexception.Parameters.Add("@DocKey", SqlDbType.BigInt);
                cmddetailexception.Parameters["@DocKey"].Value = headerid;
                adapter.Fill(myDetailExceptionTable);
            }
            ...
            myHistoryStateTable.TableName = "HistoryState";
            myDetailDeviasiTable.TableName = "DetailDeviasi";
            myDetailExceptionTable.TableName = "DetailException";
            ...
            DataColumn[] keyStateHistory = new DataColumn[1];
            keyStateHistory[0] = myHistoryStateTable.Columns["id"];
            myHistoryStateTable.PrimaryKey = keyStateHistory;

            DataColumn[] keyDetailDeviasi = new DataColumn[1];
            keyDetailDeviasi[0] = myDetailDeviasiTable.Columns["DtlKey"];
            myDetailDeviasiTable.PrimaryKey = keyDetailDeviasi;

            DataColumn[] keyDetailException = new DataColumn[1];
            keyDetailException[0] = myDetailExceptionTable.Columns["DtlKey"];
            myDetailExceptionTable.PrimaryKey = keyDetailException;
            ...
            dataSet.Tables.Add(myHistoryStateTable);
            dataSet.Tables.Add(myDetailDeviasiTable);
            dataSet.Tables.Add(myDetailExceptionTable);
            return dataSet;
        }
        ...
        protected override void SaveData(InternalMemoEntity InternalMemo, DataSet ds, string strDocName, string strUserID,  DXISaveAction saveaction, string strApprovalNote)
        {
            ...
            try
            {
                ...
                if (dataRow["DocValue"] != null)
                {
                    ...
                    switch (dataRow["DocValue"].ToString())
                    {
                        ...
                        case "7":
                            ...
                            break;
                        case "8":
                            dataRow["DocType"] = "MEMO DEVIASI";
                            break;
                        case "9":
                            dataRow["DocType"] = "MEMO EXCEPTION";
                            break;
                    }
                }
                if (saveaction == DXISaveAction.Save)
                {
                    ...
                    switch (dataRow["DocValue"].ToString())
                    {
                        ...
                        case "7":
                            ...
                            break;
                        case "8":
                            ClearDeviasiDetail(InternalMemo, saveaction);
                            SaveDataDetailDeviasi(ds, saveaction);

                            //save kalo extend deviasi
                            if (dataRow["MemoRefNo"] != null)
                            {
                                if (dataRow["MemoRefNo"].ToString() != "" && bIsNew)
                                {
                                    int iExt = GetMaxExtendDeviasi(dataRow["MemoRefNo"].ToString());
                                    dataRow["DeviasiTBO"] = iExt.ToString();
                                    InsertExtendDeviasi(dataRow["MemoRefNo"].ToString(), dataRow["DocNo"].ToString(), iExt);
                                }
                            }
                                
                            break;
                        case "9":
                            ClearExceptionDetail(InternalMemo, saveaction);
                            SaveDataDetailException(ds, saveaction);
                            break;
                    ...
                    if (saveaction == DXISaveAction.Save)
                    {
                        if (bIsNew)
                        {
                            ...
                            //klo new insert history pembuat memo
                            SaveDataHistory(strUserID, GetNamaByNIK(strUserID), Convert.ToInt32(InternalMemo.DocKey), "NEW", "");
                        }
                    }

                    //New Arga 2021/11/29 -- Save untuk fix return nya
                    if(dataRow["Status"].ToString() == "RETURN")
                    {
                        dataRow["Status"] = "NEED APPROVAL";
                        //SaveWorkingList(ds, InternalMemo, saveaction, strUserID);
                        SaveWorkingListAfterReturn(ds, InternalMemo, saveaction, strUserID);

                        //klo save return insert history pembuat memo
                        SaveDataHistory(strUserID, GetNamaByNIK(strUserID), Convert.ToInt32(InternalMemo.DocKey), "SAVE RETURN", "");
                    }
                }
                ...
                {
                    if (IsFinalApprove(InternalMemo) && saveaction == DXISaveAction.Approve && dataRow["DocValue"].ToString() == "3")
                    {
                        UpdateDataDetailPendingGiroSmile(ds, saveaction);
                    }

                    //if (IsFinalApprove(InternalMemo) && saveaction == DXISaveAction.Approve && dataRow["DocValue"].ToString() == "8")
                    //{
                    //    InsertExtendDeviasi(dataRow["MemoRefNo"].ToString(), dataRow["DocNo"].ToString(), Convert.ToInt32(dataRow["DeviasiTBO"]));
                    //}
                    ...
                    //Delete max num perpanjangan deviasi

                    if (saveaction == DXISaveAction.Reject && dataRow["DocType"].ToString() == "MEMO DEVIASI")
                    {
                        DeleteExtendDeviasi(InternalMemo.DocNo.ToString());
                    }

                    string decstate = "APPROVE";
                    if(saveaction == DXISaveAction.Reject) { decstate = "REJECT"; }
                    SaveDataHistory(strUserID, GetNamaByNIK(strUserID), Convert.ToInt32(InternalMemo.DocKey), decstate, strApprovalNote);
                }
        ...
        protected override void SaveDataApproval(DataSet ds, DXISaveAction saveaction)
        {
            ...
            try
            {
                foreach (DataRow dataRow in ds.Tables["Approval"].Rows)
                {
                    ...
                    {
                        sqlParameter6.Value = "CFO";
                    }

                    else if (dataRow.Field<string>("NIK") == "U0106546")
                    {
                        sqlParameter6.Value = "COO";
                    }

                    else if (dataRow.Field<string>("NIK") == "1906024")
                    {
                        sqlParameter6.Value = "CMO";
                    }
        ...
        protected override void SaveDataDetailPelepasanCrossColl(DataSet ds, DXISaveAction saveaction)
        {
            ...
            try
            {
                foreach (DataRow dataRow in ds.Tables["DetailPelepasanCrossColl"].Rows)
                {
                    ...
                    SqlCommand sqlCommand = new SqlCommand("INSERT INTO [dbo].[InternalMemoDetailPelepasanCrossColl] (DtlKey, DocKey, Seq, AgreementNo, AssetDesc, ValueAsset, OSPH, CicilanTenor, DendaBerjalan, AssetYear, AssetNTF) VALUES (@DtlKey, @DocKey, @Seq, @AgreementNo, @AssetDesc, @ValueAsset, @OSPH, @CicilanTenor, @DendaBerjalan, @AssetYear, @AssetNTF)");
                    sqlCommand.Connection = myconn;
                    sqlCommand.Transaction = trans;
                    ...
                    SqlParameter sqlParameter10 = sqlCommand.Parameters.Add("@AssetYear", SqlDbType.Int);
                    sqlParameter10.Value = dataRow.Field<Int32>("AssetYear");
                    sqlParameter10.Direction = ParameterDirection.Input;

                    SqlParameter sqlParameter11 = sqlCommand.Parameters.Add("@AssetNTF", SqlDbType.Decimal, 2);
                    sqlParameter11.Value = dataRow.Field<decimal>("AssetNTF");
                    sqlParameter11.Direction = ParameterDirection.Input;
        ...
        private void SaveWorkingListAfterReturn(DataSet ds, InternalMemoEntity InternalMemo, DXISaveAction saveaction, string strUserID)
        {
            SqlConnection myconn = new SqlConnection(myLocalDBSetting.ConnectionString);
            myconn.Open();
        ...
        protected bool IsFinalApprove(InternalMemoEntity InternalMemo)
        {
            bool res = false;
            string strSQL = "select Count(1) [CountNeedApprove] from InternalMemoApprovalList where IsDecision = 'F' and DocKey = ?";
            object obj = localDBSetting.ExecuteScalar(strSQL, InternalMemo.DocKey);
            if (obj != null && obj != DBNull.Value)
            {
        ...
        protected override void UpdateDataDetailPendingGiroSmile(DataSet ds, DXISaveAction saveaction)
        {
            SqlConnection myconn = new SqlConnection(myDBSetting.ConnectionString);
            myconn.Open();
            SqlTransaction trans = myconn.BeginTransaction();
            try
            {
                foreach (DataRow dataRow in ds.Tables["DetailPendingGiro"].Rows)
                {
                    ...
                    SqlCommand sqlCommand = new SqlCommand("UPDATE ACC_PDCLEASING set WARKAT_DATE=@WARKAT_DATE where CHEQNO=@CHEQNO and C_CODE=LEFT(@C_CODE,4) and CHEQSTAT<>'CN'");
                    sqlCommand.Connection = myconn;
                    sqlCommand.Transaction = trans;

                    SqlParameter sqlParameter0 = sqlCommand.Parameters.Add("@WARKAT_DATE", SqlDbType.DateTime);
                    sqlParameter0.Value = dataRow.Field<DateTime>("TglDijalankanKembali");
                    sqlParameter0.Direction = ParameterDirection.Input;
                    SqlParameter sqlParameter1 = sqlCommand.Parameters.Add("@CHEQNO", SqlDbType.VarChar);
                    sqlParameter1.Value = dataRow.Field<string>("NoGiro");
                    sqlParameter1.Direction = ParameterDirection.Input;
                    SqlParameter sqlParameter2 = sqlCommand.Parameters.Add("@C_CODE", SqlDbType.VarChar);
                    sqlParameter2.Value = dataRow.Field<string>("AgreementNo");
                    sqlParameter2.Direction = ParameterDirection.Input;
                    sqlCommand.ExecuteNonQuery();
                }
                trans.Commit();
        ...
        private string GetNamaByNIK(string NIK)
        {
            string ssql = "select top 1 DESCS from SYS_TBLEMPLOYEE where CODE = ?";
            var res = myDBSetting.GetFirstDataRow(ssql, NIK);

            return res.Field<string>("DESCS");
        ...
        private void SaveDataHistory(string strUserID, string strUserName, int iDockKey, string strState, string strNote)
        {
            ...
            try
            {
                ...
                sqlParameter3.Value = strUserName.ToUpper();
                sqlParameter3.Direction = ParameterDirection.Input;
        ...
        protected override void SaveDataDetailDeviasi(DataSet ds, DXISaveAction saveaction)
        {
            SqlConnection myconn = new SqlConnection(myLocalDBSetting.ConnectionString);
            myconn.Open();
            SqlTransaction trans = myconn.BeginTransaction();
            ...
            {
                int iRowNum = 0;
                foreach (DataRow dataRow in ds.Tables["DetailDeviasi"].Rows)
                {

                    SqlCommand sqlCommand = new SqlCommand("INSERT INTO [dbo].[InternalMemoDetailDeviasi] (DtlKey, DocKey, Seq, Perihal, Alasan, Action, Deadline) VALUES (@DtlKey, @DocKey, @Seq, @Perihal, @Alasan, @Action, @Deadline)");
                    sqlCommand.Connection = myconn;
                    sqlCommand.Transaction = trans;
                    ...
                    SqlParameter sqlParameter2 = sqlCommand.Parameters.Add("@Seq", SqlDbType.Int);
                    //sqlParameter2.Value = dataRow.Field<int>("Seq");
                    sqlParameter2.Value = iRowNum;
                    sqlParameter2.Direction = ParameterDirection.Input;
                    SqlParameter sqlParameter3 = sqlCommand.Parameters.Add("@Perihal", SqlDbType.NVarChar);
                    sqlParameter3.Value = dataRow.Field<string>("Perihal");
                    sqlParameter3.Direction = ParameterDirection.Input;
                    SqlParameter sqlParameter4 = sqlCommand.Parameters.Add("@Alasan", SqlDbType.NVarChar);
                    sqlParameter4.Value = dataRow.Field<string>("Alasan");
                    sqlParameter4.Direction = ParameterDirection.Input;
                    SqlParameter sqlParameter5 = sqlCommand.Parameters.Add("@Action", SqlDbType.NVarChar);
                    sqlParameter5.Value = dataRow.Field<string>("Action");
                    sqlParameter5.Direction = ParameterDirection.Input;
                    SqlParameter sqlParameter6 = sqlCommand.Parameters.Add("@Deadline", SqlDbType.DateTime);
                    sqlParameter6.Value = dataRow.Field<DateTime>("Deadline");
                    sqlParameter6.Direction = ParameterDirection.Input;
                    sqlCommand.ExecuteNonQuery();
        ...
        private void ClearDeviasiDetail(InternalMemoEntity InternalMemo, DXISaveAction saveaction)
        {
            SqlConnection myconn = new SqlConnection(myLocalDBSetting.ConnectionString);
            SqlCommand sqlCommand = new SqlCommand("DELETE [dbo].[InternalMemoDetailDeviasi] WHERE DocKey=@DocKey");
            sqlCommand.Connection = myconn;
            try
            {
                myconn.Open();
        ...
        protected override void SaveDataDetailException(DataSet ds, DXISaveAction saveaction)
        {
            SqlConnection myconn = new SqlConnection(myLocalDBSetting.ConnectionString);
            myconn.Open();
            SqlTransaction trans = myconn.BeginTransaction();
            try
            {
                foreach (DataRow dataRow in ds.Tables["DetailException"].Rows)
                {

                    SqlCommand sqlCommand = new SqlCommand("INSERT INTO [dbo].[InternalMemoDetailException] (DtlKey, DocKey, Seq, Perihal, Alasan, MitigasiResiko) VALUES (@DtlKey, @DocKey, @Seq, @Perihal, @Alasan, @MitigasiResiko)");
                    sqlCommand.Connection = myconn;
                    sqlCommand.Transaction = trans;
                    ...
                    sqlParameter4.Value = dataRow.Field<string>("Alasan");
                    sqlParameter4.Direction = ParameterDirection.Input;
                    SqlParameter sqlParameter5 = sqlCommand.Parameters.Add("@MitigasiResiko", SqlDbType.NVarChar);
                    sqlParameter5.Value = dataRow.Field<string>("MitigasiResiko");
                    sqlParameter5.Direction = ParameterDirection.Input;
                    sqlCommand.ExecuteNonQuery();
                }
                trans.Commit();
        ...
        private void ClearExceptionDetail(InternalMemoEntity InternalMemo, DXISaveAction saveaction)
        {
            SqlConnection myconn = new SqlConnection(myLocalDBSetting.ConnectionString);
            SqlCommand sqlCommand = new SqlCommand("DELETE [dbo].[InternalMemoDetailException] WHERE DocKey=@DocKey");
            sqlCommand.Connection = myconn;
            try
            {
                myconn.Open();
        ...
        protected int GetMaxExtendDeviasi(string strDocNo)
        {
            string ssql = "select ISNULL(MAX(PerpanjanganKe),0) + 1 as [maxExtended] from DeviasiExtendHistory where DocNo=?";
            var res = myLocalDBSetting.GetFirstDataRow(ssql, strDocNo);

            return res.Field<Int32>("maxExtended");
        }

        protected void InsertExtendDeviasi(string strDocNo, string strRefNo, int iExt)
        {
            SqlConnection myconn = new SqlConnection(myLocalDBSetting.ConnectionString);
            myconn.Open();
            ...
            try
            {
                SqlCommand sqlCommand = new SqlCommand("insert into DeviasiExtendHistory(DocNo,RefNo,PerpanjanganKe) values(@DocNo,@RefNo,@PerpanjanganKe)");
                sqlCommand.Connection = myconn;
                sqlCommand.Transaction = trans;

                SqlParameter sqlParameter1 = sqlCommand.Parameters.Add("@DocNo", SqlDbType.NVarChar, 50);
                sqlParameter1.Value = strDocNo;
                sqlParameter1.Direction = ParameterDirection.Input;
                SqlParameter sqlParameter2 = sqlCommand.Parameters.Add("@RefNo", SqlDbType.NVarChar, 50);
                sqlParameter2.Value = strRefNo;
                sqlParameter2.Direction = ParameterDirection.Input;
                SqlParameter sqlParameter3 = sqlCommand.Parameters.Add("@PerpanjanganKe", SqlDbType.Int);
                sqlParameter3.Value = iExt;
                sqlParameter3.Direction = ParameterDirection.Input;
        ...
        protected void DeleteExtendDeviasi(string strDocNo)
        {
            string ssql = "delete DeviasiExtendHistory where RefNo=?";
            myLocalDBSetting.ExecuteNonQuery(ssql, strDocNo);
        }
    }
}